...
import { getAllCategories, getProtocolCount, getProtocolsList } from "@/lib/database/queries/protocols";
...
export default async function ProtocolsPage({
  ...
  const hasFilters = Boolean(params.category || params.chain || params.q);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <p className="mt-1 text-muted-foreground">
        {hasFilters
          ? `${protocols.length} matching protocols`
          : `Top ${protocols.length} by TVL, of ${totalCount.toLocaleString()} tracked on supported chains`}
      </p>
