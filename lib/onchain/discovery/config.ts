...
export interface FactoryDeployment {
  ...
  // Only ever "uniswap-v2" this phase (see module comment above) - kept as
  // a real field, not hardcoded at every call site, so a future genuinely
  // new V2-compatible discovery shape (unlikely, since PairCreated is a
  // de facto standard every real fork reuses byte-for-byte) has somewhere
  // to attach without touching every consumer of this type.
  dexKind: "uniswap-v2";
