...
export async function syncYields() {
  ...
  console.log(`[yields] fetched ${pools.length}, ${relevant.length} relevant`);
  ...
  console.log(`[yields] synced ${rows.length} pools`);
}
...
if (require.main === module) {
  syncYields()
    .then(() => closeDb())
    .catch(async (err) => {
      console.error("[yields] sync failed:", err);
      await closeDb();
      process.exitCode = 1;
    });
}
